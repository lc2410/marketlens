...
if ($sync) {
    ...
    scp -i "$env:USERPROFILE\.ssh\github_actions" -o StrictHostKeyChecking=no ubuntu@150.136.112.120:/home/ubuntu/wallet.zip wallet.zip
    ...
    $remoteConfig = ssh -i "$env:USERPROFILE\.ssh\github_actions" -o StrictHostKeyChecking=no ubuntu@150.136.112.120 "cat /etc/systemd/system/marketlens.service"
